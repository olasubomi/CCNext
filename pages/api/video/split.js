...
export default async (req, res) => {
    ...
    if (req.method === 'POST') {
        ...
        try {
            // const p_ = await splitVideos(res.body, 'services/temp/HOW TO MAKE THE PERFECT LUMPS FREE SEMOLINA FUFU! (1).mp4')
            //   const processedFiles = await Promise.all(videoFiles.map(file =>
            //     processVideo(file.videoPath, file.timestampStart, file.timestampEnd)
            //   ));
