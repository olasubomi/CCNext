...
export const addToCart = (product) => async (dispatch, getState) => {
  try{
    console.log("product line 8 --addtocart Redux", product)
    const user = JSON.parse(localStorage.getItem("user") || "{}");

    console.log("user line 11 --addtocart Redux", user)
    if(user !== undefined && user !== "{}" && user !== null ){
      ...
      axios.post("/cart/addtocart",{ 
        ...
        console.log("cart backend", data.data.cart_items)
        console.log("cart backend", typeof data.data.cart_items)
        ...
        if(data.data){
          ...
          console.log("cart inner memo", data)
  ...
  export const removeFromCart = (productId) => async (dispatch, getState) => {
    ...
    if(user !== undefined && user !== "{}" && user !== null ){
      ...
      axios.post("/cart/removefromcart/",{ user: user, item: productId}).then(({data}) => {
  ...
  export const deleteFromCart = (productId) => (dispatch, getState) => {
    ...
    if(user !== undefined && user !== "{}" && user !== null ){
       ...
       axios.post("/cart/deletefromcart/",{ user: user, item: productId}).then(({data}) => {
        if(data){
